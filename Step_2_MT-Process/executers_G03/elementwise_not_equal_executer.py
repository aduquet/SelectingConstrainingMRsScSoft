...
def _get_outputs(all_inputs):
    ...
    try:
        inputs_outputs['td_output'] = elementwise_not_equal(all_inputs['td_a'], all_inputs['td_b'] )
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
        inputs_outputs['td_output'] = 'error'
    try:
        inputs_outputs['ttd_output_mr_per'] = elementwise_not_equal(all_inputs['td_ttd_a_MR_PER'], all_inputs['td_ttd_b_MR_PER'])
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
        inputs_outputs['ttd_output_mr_per'] = 'error'
        
    try:
        inputs_outputs['ttd_output_mr_add'] = elementwise_not_equal(all_inputs['td_ttd_a_MR_ADD'], all_inputs['td_ttd_b_MR_ADD'])
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
        inputs_outputs['ttd_output_mr_add'] = 'error'
    
    try:
        inputs_outputs['ttd_output_mr_mul'] = elementwise_not_equal(all_inputs['td_ttd_a_MR_MUL'], all_inputs['td_ttd_b_MR_MUL'])
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
        inputs_outputs['ttd_output_mr_mul'] = 'error'
    
    try:
        inputs_outputs['ttd_output_mr_inv'] = elementwise_not_equal(all_inputs['td_ttd_a_MR_INV'], all_inputs['td_ttd_b_MR_INV'])
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
        inputs_outputs['ttd_output_mr_inv'] = 'error'
    
    try:
        inputs_outputs['ttd_output_mr_inc'] = elementwise_not_equal(all_inputs['td_ttd_a_MR_INC'], all_inputs['td_ttd_b_MR_INC'])
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
        inputs_outputs['ttd_output_mr_inc'] = 'error'
    
    try:
        inputs_outputs['ttd_output_mr_exc'] = elementwise_not_equal(all_inputs['td_ttd_a_MR_EXC'], all_inputs['td_ttd_b_MR_EXC'])
    except (TypeError, ValueError, ZeroDivisionError):
        error_message = traceback.format_exc()
